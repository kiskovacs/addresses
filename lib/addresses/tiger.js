 ...
 */
function addressStream( dirPath, input ){
  var dirStream = parent.createFileAddressStreams(dirPath, inputStream, /\.shp$/, function (fileName) {
    return shapefileStream.createReadStream( fileName);
  });
  return dirStream
    .pipe( filter )
    .pipe( normalizer );
